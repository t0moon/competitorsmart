...
AI-assisted competitive intelligence toolkit for **Product Managers / PMMs**.
...
- Report output: `outputs/agent/`
- Configure API key in `.env` (for example `ZHIPU_API_KEY_1`)

For full details, refer to the Chinese section above.
...
Herramienta de inteligencia competitiva asistida por IA para **Product Managers / PMM**.
...
- Salida de reportes: `outputs/agent/`
- Configura tu clave en `.env` (por ejemplo `ZHIPU_API_KEY_1`)
...
Boite a outils d'intelligence concurrentielle assistee par IA pour **Product Managers / PMM**.
...
- Sortie des rapports: `outputs/agent/`
- Configurez votre cle dans `.env` (par exemple `ZHIPU_API_KEY_1`)
...
KI-gestuetztes Competitive-Intelligence-Toolkit fuer **Product Manager / PMM**.
...
- Berichtsausgabe: `outputs/agent/`
- API-Schluessel in `.env` setzen (z. B. `ZHIPU_API_KEY_1`)
...
**プロダクトマネージャー / PMM** 向けの AI 競合分析ツールです。
...
- `.env` に API キー（例: `ZHIPU_API_KEY_1`）を設定してください
...
**프로덕트 매니저 / PMM** 를 위한 AI 경쟁 정보 분석 도구입니다.
...
- 리포트 출력 경로: `outputs/agent/`
- `.env` 에 API 키(예: `ZHIPU_API_KEY_1`)를 설정하세요
...
Kit de inteligencia competitiva com IA para **Product Managers / PMM**.
...
- Saida dos relatorios: `outputs/agent/`
- Configure sua chave no `.env` (por exemplo `ZHIPU_API_KEY_1`)
