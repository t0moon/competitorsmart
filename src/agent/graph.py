...
工具：
  ...
  - screenshot_webpage(url) : 无头浏览器截图（PNG），写入本次运行目录供报告引用
...
from .screenshot_tool import create_screenshot_webpage_tool
...
阶段一：逐个搜集所有竞品信息（不得中途输出文字）
  ...
  3) 视觉证据（推荐）：对每家竞品尽量调用 screenshot_webpage 截取官网首页或定价页
     （full_page 按需使用）；工具返回的 Markdown 图片行须原样写入最终报告对应小节或附录；
     若工具报错（如未安装浏览器），则用文字说明并保留 URL，不要反复无效重试。
  4) 至少做 2 类外部交叉验证：评测媒体、应用商店、社区、新闻稿、招聘、案例库
  5) 必须覆盖：定位、功能、价格、用户分层、近期更新、分发渠道、商业模式、风险短板
  6) 若证据不足，继续调用工具补齐。
...
- 第四章每个竞品小节尽量包含由 screenshot_webpage 生成的插图（Markdown 图片语法，路径以工具返回为准；截图不可用时以 URL 与文字描述替代）
...
def build_graph(
    config: Config,
    *,
    screenshot_session_dir: Path,
    screenshot_md_prefix: str,
):
    """
    ...
    screenshot_session_dir: 本次运行截图 PNG 输出目录（绝对路径）。
    screenshot_md_prefix: 相对报告所在目录的截图文件夹名，用于 Markdown 引用。
    """
    screenshot_tool = create_screenshot_webpage_tool(
        screenshot_session_dir,
        screenshot_md_prefix,
    )
    tools = [search_web, fetch_webpage, screenshot_tool]
...
def run_agent(config: Config, *, export_docx: bool = False) -> str:
    """
    启动 Agent，流式打印进度，最终返回报告 Markdown 路径。
    export_docx=True 时在本机存在 pandoc 的情况下额外生成同名 .docx。
    """
    ...
    shots_prefix = f"{ts}_screenshots"
    screenshot_abs = output_dir / shots_prefix
    screenshot_abs.mkdir(parents=True, exist_ok=True)

    compiled = build_graph(
        config,
        screenshot_session_dir=screenshot_abs,
        screenshot_md_prefix=shots_prefix,
    )
                    ...
                    if hasattr(msg, "tool_calls") and msg.tool_calls:
                        for tc in msg.tool_calls:
                            ...
                            if tc["name"] == "search_web":
                                icon = "[S]"
                            elif tc["name"] == "screenshot_webpage":
                                icon = "[C]"
                            else:
                                icon = "[W]"
                            console.print(
    ...
    if export_docx:
        try:
            from src.export_docx import PandocNotFoundError, markdown_to_docx

            docx_path = markdown_to_docx(report_path)
            console.print(
                f"[bold green]✓ Word 已导出:[/bold green] [cyan]{docx_path}[/cyan]"
            )
        except PandocNotFoundError as e:
            console.print(f"[yellow]⚠ 未导出 Word：{e}[/yellow]")
        except Exception as e:
            console.print(
                f"[yellow]⚠ Word 导出失败: {type(e).__name__}: {e}[/yellow]"
            )
